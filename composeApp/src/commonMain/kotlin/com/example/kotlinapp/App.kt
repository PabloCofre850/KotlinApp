...
fun App(
    ...
    errorMessage: String?,
    onDismissError: () -> Unit,
    clienteRepository: ClienteRepository // Parámetro añadido de nuevo
) {
    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        when (pantallaActual) {
            Pantalla.LOGIN -> LoginScreen(
                ...
                clienteRepository = clienteRepository // Pasarlo aquí
            )

            Pantalla.REGISTRO -> RegisterUserScreen(
                irALogin = { onChangePantalla(Pantalla.LOGIN) },
                clienteRepository = clienteRepository // Y aquí
            )
